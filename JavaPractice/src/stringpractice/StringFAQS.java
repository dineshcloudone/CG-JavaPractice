...
public class StringFAQS {
	...
	void removeDuplicateChar(String s) {
		...
		// Using LinkedHashSet to maintain order of appearance
		Set<Character> uniqueChars = new LinkedHashSet<>();

		// Traverse the input string and add each character to the set
	...
	void binarySearch() {
		...
		if (temp) {
			System.out.println("search element: " + searchnumber + " found at index :" + String.valueOf(found_index));
		} else {
	...
	void countWord() {
		 ...
		 * 
		 * char[] ch2=name.toCharArray(); int wordcount=0; int loopcount; boolean
		 * bool=true;
		 * 
		 * for(int i=0;i<ch1.length;i++) { loopcount=0; for(int j=0;j<ch2.length;j++) {
		 * 
		 * if(ch1[i]==ch2[j]&&i<ch1.length) { i++; loopcount++;
		 * if(loopcount==name.length()) { wordcount++; } continue; }else { break; } } }
		 * 
